				...
				<tbody>
					{transactionData.map((data, index) => (
						<tr
							...
							<td className="px-6 py-4 font-medium">
								<div
									className={`px-4 py-1 rounded-full border text-center text-sm w-fit uppercase ${getStatusColor(data.status)}`}>
									{data.status}
								</div>
							</td>
							<td className="flex gap-3 px-6 py-7.5 text-gray-600">
								<button
									...
									className="flex items-center gap-2 cursor-pointer w-fit rounded-md hover:bg-gray-100 py-1 px-2">
									<FiEye size={20} />
									View Details
								</button>
							</td>
